...
public class CheckTypes extends DepthFirstVisitor
{
   ...
   public void outCallExp(CallExp node)
   {
       ...
       while(iter.hasNext() && iter2.hasNext()){
           if (!iter.next().getType().equals(iter2.next())){
...
//================================================================Other Expressions
    ...
    public void outMeggyToneStart(MeggyToneStart node){
        Type rexpType = this.mCurrentST.getExpType(node.getToneExp());
        Type lexpType = this.mCurrentST.getExpType(node.getDurationExp());
        if(lexpType == Type.TONE && (rexpType == Type.BYTE || rexpType == Type.INT)){
            this.mCurrentST.setExpType(node, Type.TONE);
        }
        else{
            throw new SemanticException(
                 "Operands must be TONE and INT OR BYTE",
            node.getLine(),
            node.getPos());
        }
    }
