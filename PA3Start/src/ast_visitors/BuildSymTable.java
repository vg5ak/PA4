...
public class BuildSymTable extends DepthFirstVisitor
{
   ...
   {
       if(this.mCurrentST.lookup(node.getName())!=null){
           throw new SemanticException(node.getName() + " " + node.getLine() + " " + node.getPos());
       }
   ...
   public void inMethodDecl(MethodDecl node)
   {
       ...
       //System.out.println("EnterMethod");
       Scope scope = this.mCurrentST.peek();
       ...
       this.mCurrentST.idlit.put(var.getName(),var);
       for(Iterator<Formal> iter = newSTE.mSignature.iterator(); iter.hasNext();){
          // System.out.println("EnterNewFormal: " + i);
           ...
           // System.out.println(var.mType.toString());

       }
       mCurrentST.mScopeStack.add(scope);//*
